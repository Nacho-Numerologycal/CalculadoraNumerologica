...
import { Calculator, X, PenLine, Eraser, RotateCcw, Palette, Calendar, ChevronDown, ChevronUp } from 'lucide-react';
...
const App = () => {
  ...
  const renderNumericalUniverse = () => {
    ...
    return (
      <CollapsibleSection title="Universo Numérico">
  ...
  const renderIdealTable = () => {
    ...
    return (
      <CollapsibleSection title="Tabla Ideal Numérica">
  ...
  const renderLifeStagesTable = () => {
    ...
    return (
      <CollapsibleSection title="Etapas Vitales">
        <div className="life-stages-table">
          <div className="stages-header">
            <div className="stage-label">Etapas:</div>
            {Object.entries(numberResults.lifeStages).map(([key, data], index) => {
              const stageNumber = index + 1;
              const ageRange = data.endAge 
                ? `${stageNumber === 1 ? '0' : numberResults.lifeStages[`stage${stageNumber-1}`].endAge + 1}/${data.endAge}`
                : `${numberResults.lifeStages.stage3.endAge + 1}/<`;
              
              return (
                <div 
                  key={key} 
                  className={`stage-circle ${data.isCurrent ? 'current-stage' : ''}`}
                >
                  {ageRange}
                </div>
              );
          ...
          <div className="stages-row">
            <div className="stage-label">Oportunidades:</div>
            {Object.entries(numberResults.lifeStages).map(([key, data], index) => {
              const stageNumber = index + 1;
              return (
          ...
          <div className="stages-row">
            <div className="stage-label">Retos:</div>
            {Object.entries(numberResults.lifeStages).map(([key, data], index) => {
              const stageNumber = index + 1;
              return (
                <div key={key} className="stage-item">
  ...
  const CollapsibleSection = ({ title, children, defaultOpen = true }) => {
    ...
    return (
      <div className={`collapsible-section ${isOpen ? 'open' : 'closed'}`}>
        ...
        <div className={`section-content ${isOpen ? 'visible' : 'hidden'}`}>
          {children}
        </div>
      </div>
    );
  ...
  const renderDateComponents = () => {
    ...
    return (
      <CollapsibleSection title="Componentes de la Fecha">
