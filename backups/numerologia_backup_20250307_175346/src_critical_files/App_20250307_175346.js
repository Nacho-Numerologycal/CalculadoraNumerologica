...
const App = () => {
  ...
  const drawLaserTrail = (ctx, x, y) => {
    // Actualizar la estela del láser
    ...
    // Dibujar la estela
    const laserCtx = canvas.getContext('2d');
    
    // No limpiar el canvas completo para no borrar los dibujos existentes
    // Solo dibujar el punto actual del láser
    laserTrail.forEach((point, index) => {
      // Solo dibujar el punto más reciente con opacidad completa
      if (index === laserTrail.length - 1) {
        laserCtx.beginPath();
        laserCtx.arc(point.x, point.y, lineWidth, 0, Math.PI * 2);
        laserCtx.fillStyle = currentColor;
        laserCtx.fill();
      }
    });
  };
