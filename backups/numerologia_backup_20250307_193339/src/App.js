...
const App = () => {
  ...
  const drawLaserTrail = useCallback(() => {
    ...
    // Crear un canvas temporal para la estela del láser
    const tempCanvas = document.createElement('canvas');
    tempCanvas.width = canvas.width;
    tempCanvas.height = canvas.height;
    const tempCtx = tempCanvas.getContext('2d');
    
    // Copiar el contenido actual del canvas al temporal
    tempCtx.drawImage(canvas, 0, 0);
    
    // Limpiar solo el canvas temporal para dibujar la estela
    tempCtx.clearRect(0, 0, tempCanvas.width, tempCanvas.height);
    
    // Dibujar la estela con opacidad decreciente en el canvas temporal
    laserTrail.forEach((point, index) => {
      ...
      tempCtx.fillStyle = `rgba(${hexToRgb(currentColor)}, ${opacity})`;
      tempCtx.fill();
      ...
      if (index === laserTrail.length - 1) {
        ...
        tempCtx.fillStyle = `rgba(${hexToRgb(currentColor)}, 0.2)`;
        tempCtx.fill();
      }
    });
    
    // Dibujar la estela del láser encima del dibujo existente
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
  ...
  const changeDrawingTool = (tool) => {
    ...
    // Si cambiamos a láser, solo inicializar la estela sin limpiar el canvas
    if (tool === 'laser') {
      setLaserTrail([]);
    }
